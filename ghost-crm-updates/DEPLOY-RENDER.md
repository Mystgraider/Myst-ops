...
1. **`frontend/nginx.conf` → `frontend/nginx.conf.template`**, using
...
2. **`.github/workflows/smoke-test.yml`** - builds and boots the full
...
2. **Run the migration** against that database before first deploy:
   ```bash
   psql "<your-connection-string>" < backend/migrations/create_wireless_networks.sql
   ```
   (or whatever migration files exist under `backend/migrations/` - check
